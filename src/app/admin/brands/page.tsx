...
export default function BrandsPage() {
  ...
  const handleEdit = (b: Brand) => { setEditing(b); setForm({ name: b.name, slug: b.slug }); };
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <h2 className="text-slate-900 font-black text-lg">Marques</h2>

      <div className="bg-white border border-gray-200 rounded-2xl p-5 space-y-4">
        <h3 className="text-slate-700 font-black text-sm">{editing ? "Modifier la marque" : "Nouvelle marque"}</h3>
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input className={inputCls} value={form.slug} onChange={(e) => setForm((f) => ({ ...f, slug: slugify(e.target.value) }))} placeholder="x96" />
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-400 text-sm">Aucune marque</div>
        ) : (
            ...
            {brands.map((b) => (
              <div key={b.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-slate-400 text-xs">{b.slug}</div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(b)} className="p-2 rounded-lg hover:bg-blue-50 text-slate-400 hover:text-blue-600 transition-colors">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                  </button>
                  <button onClick={() => handleDelete(b.id)} disabled={deleting === b.id} className="p-2 rounded-lg hover:bg-red-50 text-slate-400 hover:text-red-600 transition-colors disabled:opacity-50">
